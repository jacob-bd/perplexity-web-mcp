/* WalkMe Module */ typeof _walkmeInternals !== 'undefined' && _walkmeInternals.wmloader && _walkmeInternals.wmloader.define('wm-plugin-accessibility@4.0.6-rc.1@prelib', {"name":"wm-plugin-accessibility","version":"4.0.6-rc.1","toolbelt":"2.0.2","packageDate":"2026-01-19T09:30:37.992Z","entry":"prelib"}, function (__wmplugin__) {  
var wmbundle;
...
var getCtx = (__webpack_require__(2953)/* .getCtx */ .yK);
var tabIndexHandler = __webpack_require__(8445);
...
function Accessibility() {
	...
	function run() {
		...
		eventsListener.once('AfterSiteConfigLoaded', init);
    ...
    _this.EVENTS = {
        ...
        SHOUTOUT: {
            ...
            ShoutOutClosed : _libEvents.ShoutOutClosed,
						ShoutoutSlideChanged: "SlideChanged",
						ShoutoutDeactivateAllElements: 'ShoutoutDeactivateAllElements'
        },
        LAUNCHER: {
...
getCtx().register('DummyTabOrganizer')
    ...
    .dependencies('DefaultTextGetter, Positions, EventHandlerBindingManager, AccessibilityUtils, AccessibilityConsts, HostData, wmjQuery,' +
        'ElementEmbedderFactory, Consts, DomManager, JQuerySelectorRunner, ElementsGetter');
...
function DummyTabOrganizer(defaultTextGetter, positions, eventHandlerBindingManager, accessibilityUtils, accessibilityConsts, hostData, wmjQuery,
    ...
    function createDummyElement(deployableType, deployableId, position, pageElement, display, deployableRoles) {
        ...
        if (!dummyElement.length) {
            dummyElement = createElement(dummyElementId, pageElement);
    ...
    function createElement(dummyElementId, pageElement) {
        var tabIndex = pageElement.attr("tabindex") || 0;

        return domManager.parseElementAsJquery(
            '<div id="' + dummyElementId + '" class="walkme-to-destroy" tabindex="' + tabIndex + '" style="width: 1px;position: absolute;left: -10000px;"></div>'
        );
    }
    ...
    function onDummyElementFocus(event) {
        ...
        var deployableElement = elementsGetter.findElementWithSelector("." + getDeployableTypePrefix(deployableType) + deployableId);
    ...
    function dummyElementAction(deployableType, deployableId, actionName) {
        var dummyElement = findDummyElement(getDummyElementSelector(deployableType, deployableId));
    ...
    function findDummyElement(dummySelector, root, depth) {
        root = root || document;
        depth = depth || 1;
        if (depth > 3) return wmjQuery();

        var dummy = jQuerySelectorRunner.run(dummySelector, root);
        if (dummy.length) return dummy;

        var iframes = root.getElementsByTagName ? root.getElementsByTagName('iframe') : [];
        for (var i = 0; i < iframes.length; i++) {
            var iframe = iframes[i];
            try {
                var iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
                var found = findDummyElement(dummySelector, iframeDoc, depth + 1);
...
function FocusTracker(elementsGetter, accessibilityConsts, eventsListener, wmAccessibilityCommonUtils, wmjQuery, logger,siteConfigManager) {
    ...
    function addToStack(element, originType) {
        ...
        try {
            if (element && element.ownerDocument && element.ownerDocument.location) {
                context = element.ownerDocument;
            }
        } catch (e) {}
    ...
    function focus() {
        ...
        while(!isStackEmpty()) {
            ...
            if (!wmAccessibilityCommonUtils.isElementOnScreen(elementToFocusTo, currElementSettings.context)) {
...
function BallonCreatedAccessibilityHandler(wmAccessibilityCommonUtils, commonUtils, lib, featuresManager, stepDataGetter,
	...
	function handleBalloonCreation(event, eventData) {
		...
		if (!stepData) {
			...
			var elementForFocus = handleStepBaloon(balloon,
                wmAccessibilityCommonUtils.getRoleForDialog(_deployablesRoles.step),
	...
	function firstStepNotAvailableHandler(firstStepNotAvailable, balloon) {
        ...
        commonUtils.setRole(balloon, wmAccessibilityCommonUtils.getRoleForDialog(_deployablesRoles.step));
    ...
    function handleStepBaloon(balloonElement, stepRole, stepAriaLabel, specificAriaLabel, element, isPopup, stepId, isFocusLockEnabled) {
        ...
        if (balloonContainer) {
            ...
            balloonContainer.attr('aria-modal', 'true');

            var balloonContentSelector = '.' + _balloonContent;
            wmjQuery(balloonContentSelector).attr('aria-live', 'polite');
    ...
    function injectBetweenTabs(focusableElements, lastWMElement, firstWMElement) { 
        _keydownContexts = elementsGetter.getAllSameDomainContexts();

        _keydownContexts.forEach(function(value, context) {
            wmjQuery('body', context).on('keydown', bodyKeydownHandler);
        });
        ...
        function stopListening(e, data) {
            ...
            _keydownContexts.forEach(function(value, context) {
                wmjQuery('body', context).off('keydown', bodyKeydownHandler);
            });
...
function CaptureBalloonAccessibilityHandler(wmAccessibilityCommonUtils, wmjQuery, ariaAttributesSetter, focusBehaviorSetter) {
	...
	function init() {
		...
		_walkmeInternals.on('CaptureBalloonBodyTextClicked', handleCaptureBalloonBodyTextClicked);
	}
	...
	function setCaptureBalloonFocus(container) {
		...
		var textAreaElem = tabbableElements.filter('textarea');
		if (textAreaElem) {
			toggleTextAreaTabIndex(textAreaElem);
		}
	...
	function handleCaptureBalloonBodyTextClicked(e, params) {
		var bodyTextTextAreaElement = wmjQuery(params.element);
		toggleTextAreaTabIndex(bodyTextTextAreaElement);
	}

	function isTextAreaVisible() {
		return wmjQuery('#walkme-optional:visible').length > 0;
	}

	function toggleTextAreaTabIndex(textAreaElem) {
		var visible = isTextAreaVisible();
		wmjQuery(textAreaElem).attr('tabIndex', visible ? '0' : '-1');
	}
}
...
getCtx().register('DebuggerAccessibilityHandler')
	...
	.dependencies('WmAccessibilityCommonUtils, wmjQuery, AriaAttributesSetter, FocusBehaviorSetter, TimerManager');

function DebuggerAccessibilityHandler(wmAccessibilityCommonUtils, wmjQuery, ariaAttributesSetter, focusBehaviorSetter, timerManager) {
	var _this = this;
	_this.init = init;
...
function LauncherAccessibilityHandler(commonUtils, launcherDefaultIcon, launcherTextSetter, launcherTabOrderHandler, accessibilityConsts,
	...
	function handleLauncher(type, drawableLauncher, extraData) {
        ...
        var tabOrderHandler = elementInIframeIndicator.isInCrossDomainIframe(extraData.element, window) ? launcherTabOrderHandler.handleElementFocusByDom :
...
function ResourceAccessibilityHandler(wmAccessibilityCommonUtils, commonUtils, ariaAttributesSetter, dialogTabbaleElementsPreparer,
	...
	function handleLightBox(event, params) {
		...
		commonUtils.setRole(lightBoxElement, wmAccessibilityCommonUtils.getRoleForDialog( _deployablesRoles.resource));
...
function ShoutoutAccessibilityHandler(wmAccessibilityCommonUtils, commonUtils, ariaAttributesSetter, dialogTextSetter,
	...
	function handleContainer(component, id, uiContainer, closeButtonSelector) {
		...
		commonUtils.setRole(uiContainer, wmAccessibilityCommonUtils.getRoleForDialog(_deployablesRoles.shoutOut));
...
function ShoutoutVisualDesignAccessibilityHandler(accessibilityConsts, focusTracker, elementsGetter, wmAccessibilityCommonUtils, consts, eventsListener, featuresManager, wmAccessibilityCss, commonUtils, ariaAttributesSetter){
	...
	_this.init = init;
	var focusBehaviorSetter = getCtx().get('FocusBehaviorSetter');
	...
	function bindEvents() {
		...
		eventsListener.on(_shoutoutEvents.ShoutoutSlideChanged, handleSlideChanged);
		eventsListener.on(_shoutoutEvents.ShoutoutDeactivateAllElements, handleDeactivateAllElements);
		eventsListener.on(_shoutoutEvents.ShoutOutClosed, handleShoutoutClose);
	}

	function handleDeactivateAllElements(e, data){
		data.slides.forEach(function(slide) {
			wmjQuery(':focusable', slide).each(function(i, el) {
	...
	function handleSlideChanged(e, data) {
		[data.prevSlideIndex, data.newIndex].forEach(function(idx) {
					var isNew = idx === data.newIndex;
				wmjQuery(':focusable', data.slides[idx]).each(function(i, el) {el.setAttribute('tabindex', isNew ? '0' : '-1')});

				data.slides[idx].querySelectorAll('iframe').forEach(function(iframe) {
						if (isNew) {
		...
		var uiContainer = wmjQuery('.walkme-shoutout-' + data.shoutoutId);
		uiContainer.off('keydown');
		...
		wmAccessibilityCommonUtils.lockFocus(uiContainer, focusableElements);
		wmAccessibilityCommonUtils.announce('moved to slide number ' + (data.newIndex + 1));
	}
	...
	function handleContainer(component, id, uiContainer){
		...
		handleCarouselArrows(uiContainer);
	...
	function handleCarouselArrows(uiContainer) {
		var arrowConfig = {
			'[id^="previous-slide-button-"]': 'Previous Slide',
			'[id^="next-slide-button-"]': 'Next Slide'
		};

		uiContainer.find('.wm-visual-design-full-flex-carousel').each(function(_, carousel) {
			var parent = carousel.parentElement;

			Object.entries(arrowConfig).forEach(function(entry) {
				var selector = entry[0];
				var label = entry[1];

				parent.querySelectorAll(selector).forEach(function(arrow) {
					ariaAttributesSetter.setAriaLabel(wmjQuery(arrow), label);
				});
			});
		});
	}
...
function SmartTipFocusHandler(wmjQuery, wmAccessibilityCommonUtils, idGetter, accessibilityConsts, elementsGetter, smartTipIdentifier,
	...
	function moveFocusToIcon(smartTipId) {
		var iconElement = elementsGetter.findElementWithSelector('.' + ICON_PREFIX + smartTipId);
...
function SmartTipAccessibilityHandler(wmAccessibilityCommonUtils, commonUtils, wmjQuery, timerManager, smartTipFocusHandler, bbcodeParser,
    ...
    var TAB_KEY = accessibilityConsts.KEY_CODES.TAB;
    var ENTER_KEY = accessibilityConsts.KEY_CODES.ENTER;
    var PREFIXES = accessibilityConsts.ELEMENT_TYPES_PREFIX;
    var ICON_PREFIX = PREFIXES.smartTipIcon;
    ...
    function handleSmartTipLinkedElement(evnt, eventData) {
        ...
        if (elementInIframeIndicator.isInCrossDomainIframe(element, window)) {
            return;
        }
        ...
        if (element.is(":focusable")) {
            ...
            hoverOnFocusOrEnter(element, false);
            ...
            var smartTipType = smartTip.Info.IsWithIcon ?
                ICON_TYPE :
                TOOLTIP_TYPE;
            var display = elementsGetter.findElementWithSelector("." + PREFIXES[smartTipType] + smartTipId).css("display");
    ...
    function handleSmartTipIcon(evnt, eventData) {
        ...
        var smartTipIconElement = elementsGetter.findElementWithSelector('.' + ICON_PREFIX + smartTipId);
        ...
        if (smartTipIconElement.length !== 0) {
            ...
            if (smartTipText) {
                ...
                if (featuresManager.isFeatureEnabled('smartTipTooltipOnEnter')) {
                    wmAccessibilityCommonUtils.elementWrapper(smartTipIconElement, _deployablesAriaLabels.smartTipIcon || 'additional information');
                } else {
            ...
            var shouldHoverOnEnter = featuresManager.isFeatureEnabled('smartTipTooltipOnEnter');

            hoverOnFocusOrEnter(smartTipIconElement, shouldHoverOnEnter);

            if (shouldHoverOnEnter) {
                handleSmartTipIconElementAnnouncement(smartTipIconElement,smartTipText);
            }
    ...
    function hoverOnFocusOrEnter(element, shouldHoverOnEnter) {
        if (shouldHoverOnEnter) {
            eventHandlerBindingManager.bind('keydown', function(event) {
        ...
        var eventType = shouldHoverOnEnter ? 'click' : 'focus';
        
        eventHandlerBindingManager.bind(eventType, function() {
            element.mouseover();
        }, element);
    ...
    function handleSmartTipIconElementAnnouncement(smartTipIconElement, smartTipText) {
        eventHandlerBindingManager.bind('click', function() {
            wmAccessibilityCommonUtils.announce(smartTipText, true);
        }, smartTipIconElement);
    }
}
...
function SurveyContainerHandler(
	...
	function handleContainer(surveyElement, survey, surevyRole, surveyAriaLabel, isCommentBoxGenerated) {
		...
		commonUtils.setRole(surveyElement, wmAccessibilityCommonUtils.getRoleForDialog(surevyRole));
...
function registerKeyDown(element) {
    ...
    element.off("keydown", handleKeyboardActivation);
    element.off("keypress", handleKeyboardActivation);
    element.off("keyup", handleKeyboardActivation);

    element.on("keydown", handleKeyboardActivation);
    element.on("keypress", handleKeyboardActivation);
    element.on("keyup", handleKeyboardActivation);
}

function handleKeyboardActivation(event) {
    var element = wmjQuery(this);
		if(event && event.target && (event.target.tagName === 'TEXTAREA' || event.target.tagName === 'INPUT')) return;

    var key = event.key != null ? event.key : event.keyCode || event.which;
...
var createPluginEntry = (__webpack_require__(2953)/* .createPluginEntry */ .mJ);
...
module.exports = createPluginEntry(function(registerPlugin) {
	...
	__webpack_require__(9299);
	__webpack_require__(7893);
	...
	__webpack_require__(7319);
	__webpack_require__(5368);
	__webpack_require__(7643);
	...
	var Accessibility = (__webpack_require__(2158)/* .Accessibility */ .h);
...
function AriaAttributesSetter() {
    ...
    function setAttribute(elements, attrName, value) {
        for (var i = 0; i < elements.length; i++) {
            var currValue = elements[i].getAttribute(attrName);
...
getCtx().register('ElementsGetter')
    ...
    .dependencies('AccessibilityUtils, AccessibilityConsts, wmjQuery, SmartTipElementsGetter, JQuerySelectorRunner, FeaturesManager');

function ElementsGetter(accessibilityUtils, accessibilityConsts, wmjQuery, smartTipElementsGetter, jQuerySelectorRunner, featuresManager) {
    var _this = this;
    ...
    _this.findElementWithSelector = findElementWithSelector;
    _this.isSameDomainVisibleIframe = isSameDomainVisibleIframe;
    _this.getAllSameDomainContexts = getAllSameDomainContexts;
    ...
    function getAllTabbablesByDomOrder() {
        if (!featuresManager.isFeatureEnabled('shadowDomAccessibility')) {
            function collectTabbables(context, depth) {
                if (depth > 3) return [];
                var result = [];
                var elements = jQuerySelectorRunner.run(':tabbable, iframe, ' + getUnidentifiedTabbablesSelector(), context)
                    .filter(function() {
                        return wmjQuery(this).css("display") != "none";
                    });

                if (featuresManager.isFeatureEnabled('skipDisabledElements')) {
                    elements = elements.filter(function() {
                        return isNotDisabled(this);
                    });
                };

                elements.each(function(idx, el) {
                    if (isSameDomainVisibleIframe(el)) {
                        try {
                            var iframeTabbables = collectTabbables(el.contentDocument, depth + 1);
                            result = result.concat(iframeTabbables);
                        } catch (e) {
                        }
            ...
            var tabbables = collectTabbables(document, 1);
            return wmjQuery(tabbables);
        }
        ...
        if(! _this.lastShadowElementsUpdate ||  _this.lastShadowElementsUpdate < Date.now() - 2000) {
            ...
            if (featuresManager.isFeatureEnabled('skipDisabledElements')) {
                _this.allShadowElements = _this.allShadowElements.filter(function(el) {
                    return isNotDisabled(el);
                });
            };
    ...
    function getShadowRootActiveELement() {
        function getDeepActiveElement(element) {
            ...
            try {
                if (isSameDomainVisibleIframe(element) && element.contentDocument.activeElement) {
                        return getDeepActiveElement(element.contentDocument.activeElement);
            }} catch (e) {
                return element;
            }
            return element;
        }
        return getDeepActiveElement(document.activeElement);
    }
    ...
    function getActiveElement() {
        ...
        function getActiveElementFromIframe(element) {
            if (element && element.tagName === 'IFRAME') {
                try {
                    if (element.contentDocument && element.contentDocument.activeElement) {
                        return getActiveElementFromIframe(element.contentDocument.activeElement);
                    }
                } catch (e) {
                    return element;
                }
        ...
        return getActiveElementFromIframe(document.activeElement);
    }
    ...
    function getAllElementsIncludingShadowDOM(root) {
        ...
        function traverse(node) {
            ...
            if (isSameDomainVisibleIframe(node)) {
                try {
                    traverse(node.contentDocument.documentElement);
                } catch (e) {
                }
            }
    ...
    function isSameDomainVisibleIframe(element) {
        try {
            return (
                element &&
                element.tagName === 'IFRAME' &&
                wmjQuery(element).css("display") != "none" && 
                element.contentDocument
            )
        } catch (e) {}
    }

    function isNotDisabled(el) {
        var $el = wmjQuery(el);

        return !$el.is('[disabled="disabled"]');
    }

    function findElementWithSelector(selector, context, depth) {
        context = context || document;
        depth = depth || 1;
        if (depth > 3) return wmjQuery();

        var found = jQuerySelectorRunner.run(selector, context);
        if (found.length) return found;

        var iframes = context.getElementsByTagName ? context.getElementsByTagName('iframe') : [];
        for (var i = 0; i < iframes.length; i++) {
            var iframe = iframes[i];
            if (isSameDomainVisibleIframe(iframe)) {
                try {
                    var result = findElementWithSelector(selector, iframe.contentDocument, depth + 1);
    ...
    function getAllSameDomainContexts(root, depth, keydownContexts) {
        root = root || document;
        depth = depth || 1;
        keydownContexts = keydownContexts || new Map();

        if (depth > 3) return;
        
        keydownContexts.set(root, true);
        ...
        for (var i = 0; i < iframes.length; i++) {
            ...
            try {
                if (isSameDomainVisibleIframe(iframe)) {
                    ...
                    getAllSameDomainContexts(iframeDoc, depth + 1, keydownContexts);
                }
            } catch (e) {}
        }

        return keydownContexts;
    }
}
...
getCtx().register('FocusBehaviorSetter')
    ...
    .dependencies('wmjQuery, AccessibilityConsts, FeaturesManager, Consts, EventsListener, ElementsGetter');


function FocusBehaviorSetter(wmjQuery, accessibilityConsts, featuresManager, consts, eventsListener, elementsGetter) {
    var _this = this;
    ...
    var WALKME_ACCESSIBLE_FOCUS_BORDER = 'walkme-accessible-focus-border';
    ...
    function ctor() {
        ...
        _keydownContexts = elementsGetter.getAllSameDomainContexts();
        _useBorderFocusClass = featuresManager.isFeatureEnabled('useFocusBorder');

        if (!_enableFocusClass) {
            if (_enableOutlineOnFocusin) {
                _keydownContexts.forEach(function(value, context) {
                    wmjQuery('body', context).on("focusin", enableOutline);
                });
            } else {
                _keydownContexts.forEach(function(value, context) {
                    wmjQuery('body', context).on("keydown", enableOutline);
                });
            }
        }
    }
    ...
    function enableOutline(event) {
        ...
        if (event.which == accessibilityConsts.KEY_CODES.TAB || _enableOutlineOnFocusin) {
            ...
            _keydownContexts = elementsGetter.getAllSameDomainContexts();
            _keydownContexts.forEach(function(value, context) {
                wmjQuery('body', context).off("keydown", enableOutline)
    ...
    function disableOutline(event) {
        ...
        _keydownContexts.forEach(function(value, context) {
            wmjQuery('body', context).off("mousedown", disableOutline)
    ...
    function setFocusBehavior(focusableElements, focusClass) {
        ...
        var defaultFocusClass = WALKME_ACCESSIBLE_FOCUS_CLASS;
        if (_useBorderFocusClass) {
            defaultFocusClass = WALKME_ACCESSIBLE_FOCUS_BORDER;
        }
        focusClass = focusClass || defaultFocusClass;
        ...
        function onBlur() {
            ...
            wmjQuery(this).removeClass(focusClass)
                ...
                .removeClass(WALKME_ACCESSIBLE_FOCUS_BORDER);
        }
    }
...
function WmAccessibilityCommonUtils(commonUtils, timerManager, accessibilityUtils, focusBehaviorSetter,
    ...
    _this.getRoleForDialog = getRoleForDialog;
	...
	function isElementOnScreen(selector, context) {
		...
		var isInSameDomainIframe = wmjQuery(selector, context).length > 0;

		if (isInSameDomainIframe){
			return true;
		}
    ...
    // Allows changing the role from dialog to region on MacOS for a better VoiceOver screen reader support
    function getRoleForDialog(role) {
        if (featuresManager.isFeatureEnabled('voiceOverAccessibility') && getCtx().get('HostData').getOSData().name === 'MacOS') {
            return 'region';
        }
...
function WmAccessibilityInitializer(featuresManager, commonUtils, wmAccessibilityCss, bizFlowAccessibilityHandler, 
	...
	function initializeDeployables() {
        ...
        shoutoutVisualDesignAccessibilityHandler.init(_deployablesRoles);
...
/******/ 	var __webpack_exports__ = __webpack_require__(6834);
