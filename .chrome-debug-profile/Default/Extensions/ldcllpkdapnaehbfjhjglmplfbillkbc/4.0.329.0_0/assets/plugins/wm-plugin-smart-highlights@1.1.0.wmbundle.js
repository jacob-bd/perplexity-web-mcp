/* WalkMe Module */ typeof _walkmeInternals !== 'undefined' && _walkmeInternals.wmloader && _walkmeInternals.wmloader.define('wm-plugin-smart-highlights@1.1.0@prelib', {"name":"wm-plugin-smart-highlights","version":"1.1.0","toolbelt":"2.0.2","packageDate":"2026-03-15T14:49:32.173Z","entry":"prelib"}, function (__wmplugin__) {  
var wmbundle;
...
var log = (__webpack_require__(34)/* .log */ .R);

var _aiFetcher = null;

/**
 * Initialise the module with the shared aiFetcher instance.
 * Must be called once before any callLLM invocation.
 */
function init(aiFetcher) {
    _aiFetcher = aiFetcher;
}

/**
 * Shared LLM call wrapper used across SmartHighlights.
 *
 * @param {string} prompt          — system prompt content
 * @param {Object} [options]
 * @param {Object} [options.schema]       — JSON schema for structured output
 * @param {string} [options.funcName]     — LLM function name (default: 'freePromptMini')
 * @param {boolean} [options.ignoreFormat] — skip JSON response format and parsing
 * @returns {Promise}
 */
function callLLM(prompt, options) {
    if (!_aiFetcher) {
        log('[SmartHighlights] callLLM called before init()');
        return Promise.reject(new Error('llm.client not initialised'));
    }
    var opts = options || {};
    var schema = opts.schema || null;
    var maxTokens = opts.maxTokens || 2000;
    var funcName = opts.funcName || 'freePromptMini';
    var ignoreFormat = !!opts.ignoreFormat;
    ...
    return _aiFetcher.sendCustomLlmRequest(llmRequestBody, funcName)
        .then(function (response) {
            try {
                if (ignoreFormat) return response;
                ...
                log('[SmartHighlights] LLM JSON Parse Error: ' + e.message);
                return {};
            }
        });
}

module.exports = { init: init, callLLM: callLLM };
...
var log = (__webpack_require__(34)/* .log */ .R);
var callLLM = (__webpack_require__(959).callLLM);

var INTEGRATION_ENTITY_TYPES = {
    jira: 'jira_ticket',
    servicenow: 'snow_ticket',
    successfactors: 'successfactors_employee',
    salesforce: 'salesforce_account'
};

var INTEGRATION_DESCRIPTIONS = {
    jira: 'Jira -- search tickets by key, assignee, reporter, status, JQL',
    servicenow: 'ServiceNow -- look up tickets, list open tickets for a person',
    salesforce: 'Salesforce -- query accounts, cases, opportunities',
    successfactors: 'SuccessFactors -- look up employee profiles by name'
};

function QuestionResolver(config, integrationRunner, connectedIntegrations, cacheManager) {
    if (!(this instanceof QuestionResolver)) {
        return new QuestionResolver(config, integrationRunner, connectedIntegrations, cacheManager);
    }
    this._config = config;
    this._integrationRunner = integrationRunner;
    this._connectedIntegrations = connectedIntegrations;
    this._cacheManager = cacheManager;
}

QuestionResolver.prototype.resolve = function (data, question) {
    var self = this;
    var context = _buildDataContext(data);

    return self._classify(data, question, context).then(function (classification) {
        if (!classification || classification.action !== 'enrich') {
            return self._answerDirect(data, question, context);
        }

        var integration = classification.integration;
        var lookupKeys = classification.lookupKeys;

        if (!integration || !lookupKeys || !lookupKeys.length) {
            return self._answerDirect(data, question, context);
        }

        var cached = self._lookupCache(integration, lookupKeys);
        if (cached.length > 0) {
            log('[QuestionResolver] Cache hit for ' + integration + ': ' + lookupKeys.join(', '));
            return self._answerWithContext(data, question, context, cached);
        }

        log('[QuestionResolver] Cache miss for ' + integration + ': ' + lookupKeys.join(', ') + ' — routing to IntegrationRunner');
        return self._enrichViaRunner(data, question, context, integration).catch(function () {
            return self._answerDirect(data, question, context);
        });
    }).catch(function (err) {
        log('[QuestionResolver] Classification failed: ' + err.message + ' — falling back to direct answer');
        return self._answerDirect(data, question, context);
    });
};

QuestionResolver.prototype._classify = function (data, question, context) {
    var integrations = this._connectedIntegrations;
    var integrationLines = [];
    for (var i = 0; i < integrations.length; i++) {
        var name = integrations[i].name || integrations[i];
        var desc = INTEGRATION_DESCRIPTIONS[name];
        if (desc) integrationLines.push('- ' + name + ': ' + desc);
    }

    var prompt =
        'You are a question classifier. Given entity data and a user question, decide if the question can be answered with the existing data or needs additional data from an integration.\n' +
        '\n' +
        '## Entity Data\n' +
        context + '\n' +
        '\n' +
    ...
    return callLLM(prompt, { funcName: this._config.llm.judgeFunction});
};

QuestionResolver.prototype._lookupCache = function (integration, lookupKeys) {
    var entityType = INTEGRATION_ENTITY_TYPES[integration];
    if (!entityType || !this._cacheManager) return [];

    var results = [];
    for (var i = 0; i < lookupKeys.length; i++) {
        var cached = this._cacheManager.get(entityType, lookupKeys[i].toLowerCase());
        if (cached) {
            results.push({ type: entityType, key: lookupKeys[i], data: cached });
        }
    }
    return results;
};

QuestionResolver.prototype._answerDirect = function (data, question, context) {
    ...
    return callLLM(prompt, { funcName: this._config.llm.judgeFunction, ignoreFormat: true }).then(function (res) {
        return res || 'No answer generated.';
    });
};

QuestionResolver.prototype._answerWithContext = function (data, question, entityContext, cachedResults) {
    var enrichedParts = [];
    for (var i = 0; i < cachedResults.length; i++) {
        var r = cachedResults[i];
        enrichedParts.push('Additional data for "' + r.key + '" (' + r.type + '):\n' + JSON.stringify(r.data, null, 2));
    }

    var prompt =
        'You are a helpful assistant. Answer the question using ALL the provided information.\n\n' +
        '## Primary Entity\n' +
        entityContext + '\n\n' +
        '## Additional Context\n' +
        enrichedParts.join('\n\n') + '\n\n' +
...
QuestionResolver.prototype._enrichViaRunner = function (data, question, entityContext, integration) {
    return this._integrationRunner.run({
...
module.exports = { QuestionResolver: QuestionResolver };
...
var LS_NO_CACHE_PURGE_KEY = 'wm-sh-no-cache-purge';
...
/**
 * Wipe all in-memory entries and clear the backing storage.
 * Safe to call at any time; subsequent set() / fetchWithCache calls will
...
function purge() {
    ...
    if (!_storage) return;
    _storage.clearAll();
    log('[ApiCacheManager] Cache purged');
}

function _purgeOnFirstRun() {
    try {
        if (window.localStorage.getItem(LS_NO_CACHE_PURGE_KEY)) return;
        purge();
        window.localStorage.setItem(LS_NO_CACHE_PURGE_KEY, '1');
        log('[ApiCacheManager] First-run cache purge completed');
    } catch (e) {
        log('[ApiCacheManager] First-run purge check failed: ' + e);
    }
}
...
var DEFAULT_CONFIG = (__webpack_require__(292).DEFAULT_CONFIG);
...
function getConfig(siteConfigManager) {
    ...
    if (config && config.Custom && config.Custom.smartHighlights) {
        siteSettings = config.Custom.smartHighlights;
    }
    ...
    entities: [
        ...
        },
        {
            ...
            providerConfig: {
                ...
                oktaLoginUrl: 'https://walkme.okta.com/app/atlassian/exk1r4jfdvtyTR8Fq1t7/sso/saml?RelayState=https://walkme.atlassian.net/jira/for-you',
        ...
        },
        {
            ...
            providerConfig: {
                ...
                iframeEntryPath: '/sp',
                oktaLoginUrl: 'https://walkme.okta.com/app/servicenow_ud/exklwyp38cPassOwy1t7/sso/saml'
            }
        },
        {
            type: 'sf_case',
        ...
        },
        {
            ...
            providerConfig: {
                ...
                apiVersion: 'v54.0',
                cookieName: 'sid',
                oktaLoginUrl: 'https://walkme.okta.com/app/salesforce/exk1e93ch1oL06d7G1t7/sso/saml'
            }
        },
        {
            ...
            providerConfig: {
                ...
                iframeEntryPath: '/sf/home',
                oktaLoginUrl: 'https://walkme.okta.com/app/walkme_successfactorsproductionias_1/exk15u7x0ypzBesdL1t8/sso/saml'
            }
        }
    ],
    llm: {
...
var log = (__webpack_require__(34)/* .log */ .R);
var SF_SCHEMA = (__webpack_require__(559).SF_SCHEMA);
var callLLM = (__webpack_require__(959).callLLM);
...
var ICON_GEMINI = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 28 28" fill="none"><path d="M14 28C14 21.79 9.66 16.6 3.83 15.14C2.58 14.84 1.3 14.63 0 14.56V13.44C1.3 13.37 2.58 13.17 3.83 12.86C9.66 11.4 14 6.21 14 0C14 6.21 18.34 11.4 24.17 12.86C25.42 13.17 26.7 13.37 28 13.44V14.56C26.7 14.63 25.42 14.84 24.17 15.14C18.34 16.6 14 21.79 14 28Z" fill="url(#geminiGrad)"/><defs><linearGradient id="geminiGrad" x1="0" y1="0" x2="28" y2="28"><stop stop-color="#1C7CF4"/><stop offset="1" stop-color="#A040F0"/></linearGradient></defs></svg>';
...
var INTEGRATIONS = [
    ...
    { name: 'gemini', displayName: 'Gemini', icon: ICON_GEMINI }
];
...
function IntegrationRunner(skills, config, sfBridge) {
    if (!(this instanceof IntegrationRunner)) return new IntegrationRunner(skills, config, sfBridge);
    this._skills = skills;
    this._config = config;
...
IntegrationRunner.prototype._buildSoqlFromQuery = function (query) {
    ...
    for (var i = 0; i < filters.length; i++) {
        ...
        if (op === '=' && /name/i.test(f.field)) op = 'LIKE';
...
IntegrationRunner.prototype._runSoqlIntegration = function (prompt, userContext) {
    ...
    return callLLM(schemaPrompt, { funcName: self._config.llm.nerFunction }).then(function (intent) {
        if (!intent || !Array.isArray(intent.queries) || !intent.queries.length) {
            return 'I couldn\'t find a specific Salesforce query for that. Could you provide more detail — like a company name, case number, or what you\'re looking for?';
        }
        ...
        return Promise.all(fetchPromises).then(function (results) {
            ...
            return callLLM(answerPrompt, { funcName: self._config.llm.judgeFunction, maxTokens:4000}).then(function (res) {
...
IntegrationRunner.prototype.run = function (options) {
    ...
    if (integrationName === 'gemini') {
        return self._runGemini(prompt);
    }
...
IntegrationRunner.prototype._runJira = function (prompt, userContext) {
    ...
    return callLLM(jqlPrompt, { funcName: self._config.llm.nerFunction }).then(function (result) {
...
IntegrationRunner.prototype._runGeneric = function (integrationName, prompt, userContext, skillNames) {
    ...
    return callLLM(intentPrompt, { funcName: self._config.llm.nerFunction }).then(function (intent) {
        if (!intent || !intent.skill) return 'I wasn\'t sure what to look up. Could you be more specific — like a ticket number, person name, or what you need?';
...
IntegrationRunner.prototype._getCustomAI = function () {
    var ctx = window._walkmeInternals && window._walkmeInternals.ctx;
    return ctx && ctx.has('CustomAIFactory') ? ctx.get('CustomAIFactory') : null;
};

IntegrationRunner.prototype._runGemini = function (prompt) {
    var customAI = this._getCustomAI();
    if (!customAI || !customAI.isAvailable()) {
        log('[SmartHighlights] Gemini not available');
        return Promise.reject(new Error('Gemini is not available'));
    }
    log('[SmartHighlights] runGemini: prompt="' + prompt + '"');
    return customAI.customAIPrompt(prompt).then(function (response) {
        return response || 'No response from Gemini.';
    }).catch(function (err) {
        log('[SmartHighlights] Gemini error: ' + (err && err.message || err));
...
var getCtx = (__webpack_require__(346)/* .getCtx */ .yK);
...
var createPluginEntry = (__webpack_require__(346)/* .createPluginEntry */ .mJ);

module.exports = createPluginEntry(function(registerPlugin) {
    var SmartHighlights = (__webpack_require__(560).SmartHighlights);
...
function DatapulseBridge(config) {
    if (!(this instanceof DatapulseBridge)) return new DatapulseBridge(config);
    this._config = config;
}
...
DatapulseBridge.prototype.validate = function (emailText, candidates) {
    ...
    return callLLM(fullPrompt, { schema: schema, funcName: this._config.llm.judgeFunction }).then(function (response) {
...
JiraBridge.prototype.openLoginPopup = function () {
    ...
    var url = self._providerConfig.oktaLoginUrl || (domain + loginPath);
    var w = 300, h = 300;
    var l = window.screenX + (window.outerWidth / 2) - (w / 2);
    var t = window.screenY + (window.outerHeight / 2) - (h / 2);
...
var FETCH_TIMEOUT_MS = 10000;
var AUTH_TIMEOUT_MS = 15000;
var OKTA_AUTH_TIMEOUT_MS = 30000;
...
SalesforceBridge.prototype._handleAuthRequired = function () {
    ...
    var oktaUrl = self._providerConfig.oktaLoginUrl;
    if (oktaUrl && !self._oktaAttempted) {
        self._oktaAttempted = true;
        var domain = self._providerConfig.domain;
        var entryPath = self._providerConfig.iframeEntryPath;
        var frame = document.getElementById(IFRAME_ID);
        if (frame) {
            log('[SF Master] Auth required — redirecting iframe to Okta SSO: ' + oktaUrl);
            frame.style.borderColor = "blue";

            var redirectDone = false;
            var settleTimer = null;

            function doClassicRedirect() {
                if (redirectDone) return;
                redirectDone = true;
                frame.removeEventListener('load', onOktaLoad);
                log('[SF Master] Okta SAML chain settled — redirecting iframe to classic: ' + domain + entryPath);
                frame.style.borderColor = "cyan";
                frame.src = domain + entryPath;
            }

            function onOktaLoad() {
                log('[SF Master] Okta redirect: iframe load event fired');
                if (self._isWorkerReady || redirectDone) return;
                if (settleTimer) clearTimeout(settleTimer);
                settleTimer = setTimeout(doClassicRedirect, 8000);
            }

            frame.addEventListener('load', onOktaLoad);
            frame.src = oktaUrl;
        }
        self._authTimer = setTimeout(function () {
            if (!self._isWorkerReady) {
                log('[SF Master] Okta SSO did not result in worker ready after ' + OKTA_AUTH_TIMEOUT_MS + 'ms — auth required');
                self._finalizeAuthRequired();
            }
        }, OKTA_AUTH_TIMEOUT_MS);
        return;
    }
...
SalesforceBridge.prototype._finalizeAuthRequired = function () {
    this._authRequired = true;
    this._isWorkerReady = false;
    var frame = document.getElementById(IFRAME_ID);
    if (frame) frame.style.borderColor = "red";
...
SalesforceBridge.prototype.openLoginPopup = function () {
    ...
    var domain = this._providerConfig.domain;
    var entryPath = this._providerConfig.iframeEntryPath;
    var loginUrl = self._providerConfig.oktaLoginUrl || (domain + '/lightning/page/home');
    var w = 600, h = 700;
...
function SalesforceProvider(bridge, providerConfig, config, cacheManager) {
    if (!(this instanceof SalesforceProvider)) return new SalesforceProvider(bridge, providerConfig, config, cacheManager);
    this._bridge = bridge;
    this._providerConfig = providerConfig;
    this._config = config;
    this._cacheManager = cacheManager || null;
...
SalesforceProvider.prototype.validateAccounts = function (contextText, searchedNames, records) {
    ...
    return callLLM(fullPrompt, { schema: schema, funcName: judgeFunc }).then(function (response) {
...
SnowBridge.prototype._handleAuthRequired = function () {
    ...
    if (oktaUrl && !self._oktaAttempted) {
        ...
        var frame = document.getElementById(IFRAME_ID);
        if (frame) {
            log('[SNOW Master] Auth required — redirecting iframe to Okta SSO: ' + oktaUrl);
            frame.style.borderColor = "blue";
            frame.src = oktaUrl;
        }
        self._authTimer = setTimeout(function () {
            if (!self._isWorkerReady) {
                log('[SNOW Master] Okta SSO did not result in worker ready after ' + OKTA_AUTH_TIMEOUT_MS + 'ms — auth required');
                self._finalizeAuthRequired();
            }
        }, OKTA_AUTH_TIMEOUT_MS);
        return;
...
SnowBridge.prototype._finalizeAuthRequired = function () {
    this._authRequired = true;
    this._isWorkerReady = false;
    var frame = document.getElementById(IFRAME_ID);
    if (frame) frame.style.borderColor = "red";
...
SnowBridge.prototype.openLoginPopup = function () {
    ...
    var l = (screen.width / 2) - (w / 2);
    var t = (screen.height / 2) - (h / 2);
    var loginUrl = self._providerConfig.oktaLoginUrl || (domain + entryPath);
    log('[SNOW Master] Opening login popup: ' + loginUrl);
    var popup = window.open(loginUrl, 'SnowLogin', 'width=' + w + ',height=' + h + ',top=' + t + ',left=' + l);
...
var AUTH_TIMEOUT_MS = 15000;
var OKTA_AUTH_TIMEOUT_MS = 10000;
var KEEPALIVE_MS = 5 * 60 * 1000;
...
SapBridge.prototype._handleAuthRequired = function () {
    ...
    var oktaUrl = self._providerConfig.oktaLoginUrl;
    if (oktaUrl && self._oktaAttempts < 2) {
        self._oktaAttempts++;
        var frame = document.getElementById(IFRAME_ID);
        if (frame) {
            log('[SAP Master] Auth required — Okta SSO attempt ' + self._oktaAttempts + '/2: ' + oktaUrl);
            frame.style.borderColor = self._oktaAttempts === 1 ? "blue" : "yellow";
            frame.src = oktaUrl;
        }
        self._authTimer = setTimeout(function () {
            if (!self._isWorkerReady) {
                log('[SAP Master] Okta SSO attempt ' + self._oktaAttempts + ' did not complete after ' + OKTA_AUTH_TIMEOUT_MS + 'ms');
                self._handleAuthRequired();
            }
        }, OKTA_AUTH_TIMEOUT_MS);
        return;
...
SapBridge.prototype._finalizeAuthRequired = function () {
    this._authRequired = true;
    this._isWorkerReady = false;
    var frame = document.getElementById(IFRAME_ID);
    if (frame) frame.style.borderColor = "red";
...
SapBridge.prototype.openLoginPopup = function () {
    ...
    var t = (screen.height / 2) - (h / 2);
    var loginUrl = self._providerConfig.oktaLoginUrl || (domain + entryPath);
    log('[SAP Master] Opening login popup: ' + loginUrl);
    var popup = window.open(loginUrl, 'SapLogin', 'width=' + w + ',height=' + h + ',top=' + t + ',left=' + l);
...
function NerScanner(config, cache) {
    if (!(this instanceof NerScanner)) return new NerScanner(config, cache);
    this._config = config;
    this._cache = cache;
}
...
NerScanner.prototype.scan = function (text) {
    ...
    return callLLM(prompt, { schema: schema, funcName: this._config.llm.nerFunction }).then(function (response) {
        var cleanedResponse = {};
...
var getCtx = (__webpack_require__(346)/* .getCtx */ .yK);
var log = (__webpack_require__(34)/* .log */ .R);
var platformUtils = __webpack_require__(340);
var textUtils = __webpack_require__(148);
var ICONS = (__webpack_require__(535).ICONS);
var configModule = __webpack_require__(749);
var apiCacheManager = __webpack_require__(952);
var IdbStorageProvider = (__webpack_require__(628).IdbStorageProvider);
var NerScanner = (__webpack_require__(236).NerScanner);
var RegexScanner = (__webpack_require__(522).RegexScanner);
var Scanner = (__webpack_require__(751).Scanner);
var DatapulseBridge = (__webpack_require__(899).DatapulseBridge);
var DatapulseProvider = (__webpack_require__(727).DatapulseProvider);
var JiraAuth = (__webpack_require__(838).JiraAuth);
var JiraBridge = (__webpack_require__(869).JiraBridge);
var JiraProvider = (__webpack_require__(837).JiraProvider);
var SnowAuth = (__webpack_require__(814).SnowAuth);
var SnowBridge = (__webpack_require__(933).SnowBridge);
var SnowProvider = (__webpack_require__(277).SnowProvider);
var SalesforceAuth = (__webpack_require__(658).SalesforceAuth);
var SalesforceBridge = (__webpack_require__(121).SalesforceBridge);
var SalesforceProvider = (__webpack_require__(433).SalesforceProvider);
var SalesforceCaseProvider = (__webpack_require__(889).SalesforceCaseProvider);
var SalesforceHarmonyOpportunityProvider = (__webpack_require__(900).SalesforceHarmonyOpportunityProvider);
var SalesforceContactProvider = (__webpack_require__(587).SalesforceContactProvider);
var SapAuth = (__webpack_require__(807).SapAuth);
var SapBridge = (__webpack_require__(536).SapBridge);
var SapProvider = (__webpack_require__(828).SapProvider);
var DemoProvider = (__webpack_require__(171).DemoProvider);
var IntegrationRunner = (__webpack_require__(958).IntegrationRunner);
var QuestionResolver = (__webpack_require__(253).QuestionResolver);
var llmClient = __webpack_require__(959);
var Balloon = (__webpack_require__(260).Balloon);
var Highlight = (__webpack_require__(647).Highlight);
var NativeCards = (__webpack_require__(690).NativeCards);
...
(function earlyWorkerInit() {
    if (window.parent === window) {
        ...
        if (window.name === 'SfLogin' && platformUtils.isSalesforceDomain()) {
            ...
            (function sfTryAutoLogin() {
                var oktaBtn = document.querySelector('#idp_hint button.secondary.wide');
    ...
    if (platformUtils.isSalesforceDomain()) {
        ...
        if (sfEnt && sfEnt.providerConfig) {
            ...
            var sfIframeAttempts = 0;
            (function sfIframeTryOktaLogin() {
                var oktaBtn = document.querySelector('#idp_hint button.secondary.wide');
                if (oktaBtn && oktaBtn.textContent.indexOf('Okta') > -1) {
                    log('[EarlyWorker] SF iframe — clicking Okta login button');
                    oktaBtn.click();
                    return;
                }
                if (++sfIframeAttempts < 20) {
                    setTimeout(sfIframeTryOktaLogin, 500);
                }
            })();
        }
    }
...
function SmartHighlights() {
    ...
    function run() {
        ...
        eventsListener.once('WalkMeReady', function () {
            ...
            var jiraSegmentResult = evaluateSegments(pluginConfig.jiraSegments);
            if (jiraSegmentResult === 'match') {
                isJiraEnabled = true;
                log('[SmartHighlights] Jira enabled — segment match');
            } else if (jiraSegmentResult === 'disabled') {
                log('[SmartHighlights] Jira segment disabled (-1)');
            } else if (jiraSegmentResult === 'no_match') {
                log('[SmartHighlights] User does not match any Jira segment — Jira disabled');
            } else {
                log('[SmartHighlights] Jira segments not configured — Jira disabled');
            }
            try {
                if (localStorage.getItem('sh-use-jira') === '1') {
                    isJiraEnabled = true;
                    log('[SmartHighlights] Jira enabled via localStorage override');
                }
            } catch (e) {}

            var sfdcSegmentResult = evaluateSegments(pluginConfig.sfdcSegments);
            if (sfdcSegmentResult === 'match') {
                isSalesforceIntegrationEnabled = true;
                log('[SmartHighlights] Salesforce enabled — segment match');
            ...
            } else if (sfdcSegmentResult === 'disabled') {
                log('[SmartHighlights] Salesforce segment disabled (-1)');
            } else if (sfdcSegmentResult === 'no_match') {
                log('[SmartHighlights] User does not match any Salesforce segment — Salesforce disabled');
            } else {
                log('[SmartHighlights] Salesforce segments not configured — Salesforce disabled');
            }
            try {
                if (localStorage.getItem('sh-use-sfdc') === '1') {
                    isSalesforceIntegrationEnabled = true;
                    for (var lsci = 0; lsci < pluginConfig.entities.length; lsci++) {
                        if (pluginConfig.entities[lsci].ner === 'Company Names') {
                            pluginConfig.entities[lsci].provider = 'salesforce';
                            break;
                        }
                    }
                    log('[SmartHighlights] Salesforce enabled via localStorage override');
                }
            } catch (e) {}

            if (!isJiraEnabled) {
                pluginConfig.entities = pluginConfig.entities.filter(function (e) { return e.type !== 'jira_ticket'; });
            }
            if (!isSalesforceIntegrationEnabled) {
                pluginConfig.entities = pluginConfig.entities.filter(function (e) {
                    return e.type !== 'sf_case' && e.type !== 'sf_harmony_opportunity';
                });
            }
            ...
            nerScanner = new NerScanner(pluginConfig, cache);
            ...
            datapulseBridge = new DatapulseBridge(pluginConfig);
            ...
            sfProvider = new SalesforceProvider(sfBridge, sfPC, pluginConfig, apiCacheManager);
            ...
            if (platformUtils.isJiraDomain()) { jiraBridge.initWorker(); addConnectedIntegration('jira'); }
                ...
                if (isJiraEnabled) jiraBridge.initMaster();
                snowBridge.initMaster();
                if (isSalesforceIntegrationEnabled) sfBridge.initMaster();
            ...
            if (platformUtils.isGmailDomain()) {
                var wmCtx = window._walkmeInternals && window._walkmeInternals.ctx;
                if (wmCtx && wmCtx.has('CustomAIFactory') && wmCtx.get('CustomAIFactory').isAvailable()) {
                    addConnectedIntegration('gemini');
                }
            }

            balloon = new Balloon(pluginConfig, ICONS, serverLog, handleManagerClick);
            ...
            var integrationRunner = new IntegrationRunner(_this.skills, pluginConfig, sfBridge);
            var questionResolver = new QuestionResolver(pluginConfig, integrationRunner, connectedIntegrations, apiCacheManager);
            balloon._questionResolver = questionResolver;
    ...
    function getSiteSettings(siteConfigManager) {
        ...
        if (cfg && cfg.Custom && cfg.Custom.smartHighlights) return cfg.Custom.smartHighlights;
        return {};
    }
    ...
    function evaluateSegments(segments) {
        if (!segments || !Array.isArray(segments) || segments.length === 0) return 'not_configured';
        if (segments.length === 1 && segments[0] === -1) return 'disabled';
        for (var i = 0; i < segments.length; i++) {
            if (WalkMeAPI.evaluateSegmentTag(segments[i])) return 'match';
        }
        return 'no_match';
    }
    ...
    function tryFetchAndHighlight() {
        ...
        scanner.scan(trimmed).then(function (scanResults) {
            ...
            if (isJiraEnabled && jiraKeys.length > 0 && !(platformUtils.isJiraDomain() && window.top === window.self)) {
...
var log = (__webpack_require__(34)/* .log */ .R);
var textUtils = __webpack_require__(148);
var createPaginationTop = (__webpack_require__(151).createPaginationTop);
var createFollowUp = (__webpack_require__(7).createFollowUp);
var domHelpers = __webpack_require__(206);
var renderQuestionView = (__webpack_require__(302).renderQuestionView);
var renderLoginView = (__webpack_require__(615).renderLoginView);
var callLLM = (__webpack_require__(959).callLLM);

var providerIcons = __webpack_require__(304);
...
function Balloon(config, icons, serverLog, onManagerClick) {
    if (!(this instanceof Balloon)) return new Balloon(config, icons, serverLog, onManagerClick);
    this._config = config;
    this._icons = icons;
...
Balloon.prototype.generateSummary = function (data) {
    ...
    return callLLM(prompt, { funcName: this._config.llm.judgeFunction }).then(function (res) {
...
Balloon.prototype.handleAskQuestionClick = function (data, question, currentContentArray) {
    ...
    var qPromise = this._questionResolver
        ? this._questionResolver.resolve(data, question)
        : this.generateAnswer(data, question);
...
Balloon.prototype.show = function (highlightElement, contentArray) {
    ...
    if (balloon._loginInProgressKey) return;
...
var BALLOON_CSS = (__webpack_require__(645).BALLOON_CSS);
...
Highlight.prototype._createHighlightedBalloon = function (searchString, contentArray) {
    ...
    if (balloon._loginInProgressKey &&
        balloon._loginInProgressKey.trim().toLowerCase() === searchString.trim().toLowerCase() &&
        contentArray[0] && contentArray[0].type !== 'jira_auth' && contentArray[0].type !== 'snow_auth' &&
        contentArray[0].type !== 'sap_auth' && contentArray[0].type !== 'sf_auth') {
        balloon._loginInProgressKey = null;
        self._balloon._oldContentArray = null;
        self._balloon.renderBalloonContent(contentArray);
    }
...
function _buildLoginButton(data, provider, opts) {
    ...
    btn.onclick = function (e) {
        ...
        if (balloon) {
            balloon._loginInProgressKey = data.title;
            while (balloon.firstChild) balloon.removeChild(balloon.firstChild);
            var wrapper = document.createElement('div');
            wrapper.className = 'wm-login-wrapper';
            wrapper.appendChild(_buildHeader(data, provider, opts));
            ...
            var pulseWrapper = document.createElement('div');
            pulseWrapper.className = 'wm-loading-pulse';
            pulseWrapper.appendChild(opts.getLargeLogo());
            center.appendChild(pulseWrapper);
            var loadingText = document.createElement('div');
            loadingText.className = 'wm-login-message';
            loadingText.innerText = 'Getting things ready for you\u2026';
            center.appendChild(loadingText);
            wrapper.appendChild(center);
            balloon.appendChild(wrapper);
        }

        if (opts.onAfterLogin) opts.onAfterLogin();
    };
...
function isOktaDomain() {
    return window.location.hostname.indexOf('okta.com') > -1;
}
